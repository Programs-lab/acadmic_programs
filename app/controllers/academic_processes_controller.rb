class AcademicProcessesController < ApplicationController
  ...
  before_action :set_process, only: [:edit, :update, :destroy]
  def index
  ...
  def academic_process_params
    params.require(:academic_process).permit(:name, :year_saces, :month_saces, :day_saces, :year_academic_council, :month_academic_council, :day_academic_council)
  end
